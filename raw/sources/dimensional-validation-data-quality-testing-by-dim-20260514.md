---
type: clip
title: "Dimensional Validation | Data Quality Testing by Dimension - OpenMetadata Documentation"
url: "https://docs.open-metadata.org/v1.12.x/how-to-guides/data-quality-observability/quality/dimensional-validation"
clipped: 2026-05-14
origin: web-clip
sources: []
tags: [web-clip]
...
# Dimensional Validation | Data Quality Testing by Dimension - OpenMetadata Documentation

Source: https://docs.open-metadata.org/v1.12.x/how-to-guides/data-quality-observability/quality/dimensional-validation

Skip to main contentOpenMetadata Documentation home pagev1.12.xSearch...⌘KSearch...NavigationData QualityDimensional Validation | Data Quality Testing by DimensionHomeQuickstartDeploymentConnectorsHow-to GuidesMCPAdmin GuideGuide for Data UsersData DiscoveryData ContractsData CollaborationData Quality and ObservabilityData Quality and ProfilerData Quality ObservabilityData LineageData InsightsData GovernanceReleasesDevelopersSDK & APIsData Quality and ObservabilityOverviewData QualityOverviewConfigure Data QualityTests - YAML ConfigColumn Tests - YAML ConfigTests - UI ConfigDimensional ValidationData Quality as CodeCustom TestsTest LibraryData ProfilerAlerts & NotificationsIncident ManagerOn this pageDimensional ValidationWhat is Dimensional Validation?Key ConceptsDimension ColumnDimension GroupTop Dimensions”Others” GroupCardinalityWhen to Use Dimensional ValidationMulti-Region or Multi-Location DataProduct or Category AnalysisCustomer Segment QualityWhen NOT to Use Dimensional ValidationQuick Start GuideStep 1: Navigate to Your TableStep 2: Select Column and Test TypeStep 3: Finish configuring your test, pipeline and running itStep 4: View ResultsUnderstanding ResultsResult MetricsImpact Score RankingThe “Others” GroupHistorical Trend ViewBest PracticesChoose Meaningful DimensionsOptimize CardinalityStart Small, Then ScaleCombine with Sampling for Large TablesUse PartitioningMonitor Test Execution TimeName Tests ClearlyDocument Business ContextReal-World ExamplesExample 1: E-Commerce Product Data QualityExample 2: Multi-Region Customer Email ValidationExample 3: Financial Transaction Amount MonitoringLimitationsHigh Cardinality Performance Impact”Others” Group LimitationsTroubleshootingMy test returns “No results”Dimensional results show “NULL” as a dimension valueTest execution takes too longDocumentation IndexFetch the complete documentation index at: https://docs.open-metadata.org/llms.txtUse this file to discover all available pages before exploring further.​Dimensional Validation
...
Go to the table where you want to create a dimensional test

Click on the Data Observability tab

Click the Add Test button in the upper right corner

Select Dimension Level

​Step 2: Select Column and Test Type

Choose your target column: Select the column you want to validate (e.g., customer_email, order_amount, product_description)

Select the dimension: Select the dimension you are interested in (e.g, region, status)

Select a test type: Choose from any of the supported column-level tests (see Supported Test Types section)

Configure test parameters: Set thresholds, ranges, or patterns based on your chosen test type
...
For detailed information on setting up tests, see Adding Test Cases to an Entity and Adding Test Suites

​Step 4: View Results

Once your test runs, return to the Data Observability tab to view dimensional results:

Find your dimensional test in the list

Click on the test to expand results

View the Dimensional Results section showing metrics for each dimension group
...
Failure rate: What percentage of rows failed in this dimension?

Absolute volume: How many rows failed in total?
...
Start with one or two critical tests on important tables

Choose low-cardinality dimensions initially

Validate performance before expanding to more tests

Add sampling if execution time exceeds acceptable limits

Gradually expand to additional tables and dimensions

​Combine with Sampling for Large Tables

For large tables, enable Profile Sample to reduce data scan volumes

For detailed sampling configuration, see Profiler Workflow - Profile Sample.

​Use Partitioning

For large tables with partitioning columns, enable partitioning to focus tests on meaningful data

For detailed partitioning configuration, see Profiler Workflow - Enable Partition.
...
Range: 0.01to0.01 to 0.01to10,000
...
Enable partitioning to test only recent data

Choose a different dimension with lower cardinality

Was this page helpful?YesNoSuggest editsRaise issueTests - UI Config | OpenMetadata Quality Config GuidePreviousData Quality as CodeNext⌘I
