...
source: https://duckdb.org/2022/09/30/postgres-scanner?s=09
author:
  - "[[Hannes Mühleisen]]"
published: 2022-09-30
created: 2026-04-04
description: DuckDB can now directly query tables stored in PostgreSQL and speed up complex analytical queries without duplicating data.
tags:
  - clippings
  - duckdb
topic:
...
![DuckDB goes Postgres](https://duckdb.org/images/blog/elephant-duck.jpg)
...
To allow for fast and consistent analytical reads of Postgres databases, we designed and implemented the "Postgres Scanner". This scanner leverages the *binary transfer mode* of the Postgres client-server protocol (See the [Implementation Section](https://duckdb.org/2022/09/30/postgres-scanner?s=09#implementation) for more details.), allowing us to efficiently transform and use the data directly in DuckDB.

Among other things, DuckDB's design is different from conventional data management systems because DuckDB's query processing engine can run on nearly arbitrary data sources without needing to copy the data into its own storage format. For example, DuckDB can currently directly run queries on [Parquet files](https://duckdb.org/docs/lts/data/parquet/overview.html), [CSV files](https://duckdb.org/docs/lts/data/csv/overview.html), [SQLite files](https://github.com/duckdb/duckdb-sqlite), [Pandas](https://duckdb.org/docs/lts/guides/python/sql_on_pandas.html), [R](https://duckdb.org/docs/lts/clients/r.html#efficient-transfer) and [Julia](https://duckdb.org/docs/lts/clients/julia.html#scanning-dataframes) data frames as well as [Apache Arrow sources](https://duckdb.org/docs/lts/guides/python/sql_on_arrow.html). This new extension adds the capability to directly query PostgreSQL tables from DuckDB.
