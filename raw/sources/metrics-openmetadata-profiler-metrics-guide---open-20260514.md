---
type: clip
title: "Metrics | OpenMetadata Profiler Metrics Guide - OpenMetadata Documentation"
...
# Metrics | OpenMetadata Profiler Metrics Guide - OpenMetadata Documentation

Source: https://docs.open-metadata.org/v1.12.x/how-to-guides/data-quality-observability/profiler/metrics

Skip to main contentOpenMetadata Documentation home pagev1.12.xSearch...⌘KSearch...NavigationData ProfilerMetrics | OpenMetadata Profiler Metrics GuideHomeQuickstartDeploymentConnectorsHow-to GuidesMCPAdmin GuideGuide for Data UsersData DiscoveryData ContractsData CollaborationData Quality and ObservabilityData Quality and ProfilerData Quality ObservabilityData LineageData InsightsData GovernanceReleasesDevelopersSDK & APIsData Quality and ObservabilityOverviewData QualityData ProfilerOverviewProfiler WorkflowMetricsCustom MetricsExternal workflowAlerts & NotificationsIncident ManagerOn this pageProfiler MetricsTable MetricsRow CountColumn CountSystem MetricsDML OperationsRows Affected by the DML OperationColumn MetricsValues CountValues PercentageDuplicate CountNull CountNull ProportionUnique CountUnique ProportionDistinct CountDistinct ProportionMinMaxMin LengthMax LengthMeanMedianSumStandard DeviationHistogramFirst QuartileThird QuartileInter Quartile RangeNonparametric SkewGrant Access to User for System MetricsSnowflakeRedshiftBigQueryReach out!Documentation IndexFetch the complete documentation index at: https://docs.open-metadata.org/llms.txtUse this file to discover all available pages before exploring further.​Profiler Metrics
...
You can check the definition of the columnProfile here. On the other hand, the metrics are implemented here.
...
S=μ−μ~σ  S = \frac{\mu-\tilde{\mu}}{\sigma}S=σμ−μ~​​

Where

μ=meanμ~=medianσ=standarddeviation\mu = mean\\

\tilde{\mu} = median\\

\sigma = standard deviation\\μ=meanμ~​=medianσ=standarddeviation
...
Is there any metric you’d like to see? Open an issue or reach out on Slack.Was this page helpful?YesNoSuggest editsRaise issueProfiler Workflow | OpenMetadata Profiling WorkflowPreviousCustom MetricsNext⌘I
